...
> 페이지 사이 2.5초 간격으로 탭 하나만 사용합니다(카테고리당 최대 20페이지, 상세 최대 500건).
> 쿠팡이 확인 화면(CAPTCHA)이나 차단을 띄우면 **우회하지 않고** 그 대상만 실패로 기록한 뒤 다음으로
> 넘어가며, 5회 연속 실패하면 스스로 멈춥니다. 실패 사유는 DB(`scan_targets.error`)와 확장 콘솔에 남습니다.
