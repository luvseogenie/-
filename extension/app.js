...
// 캠페인 옵션 목록 + 현재 마진 + 최근 60일 평균 판매가 (마진을 옵션마다 바꿀 때)
function campOptionRows(c) {
  const lookup = S.marginLookup(DATA); const t = localIso(today); const since = addDays(localIso(yday), -59); const names = S.productNames(DATA);
  const opts = DATA.options.filter((o) => o.campaign === c); const ids = new Set(opts.map((o) => o.option_id)); const rev = {}, qty = {}; const shortNames = S.shortOptionNames(DATA);
  for (const [date, day] of Object.entries(DATA.sales)) { if (date < since) continue; for (const r of Object.values(day)) if (ids.has(r.option_id) && r.quantity > 0) { rev[r.option_id] = (rev[r.option_id] || 0) + (r.revenue || 0); qty[r.option_id] = (qty[r.option_id] || 0) + r.quantity; } }
  return opts.map((o) => { const hist = S.marginHistory(DATA, o.option_id); const cur = [...hist].reverse().find((h) => h.effective_from <= t) || null; return { option_id: o.option_id, name: shortNames[o.option_id] || o.product_name || names[o.option_id] || '', full: o.product_name || names[o.option_id] || '', margin: lookup(o.option_id, t), since: cur ? (cur.effective_from || '처음부터') : '', note: cur?.note || '', hist, price: qty[o.option_id] ? rev[o.option_id] / qty[o.option_id] : 0 }; });
}
...
function renderRoasCheck() {
  ...
  const setN = rows.filter((r) => r.zeroSource === 'manual').length, unsetN = rows.length - setN;
  $('#roas-summary').innerHTML = `<span class="pill bad">조정 필요 ${n('red')}</span> <span class="pill good">적정 ${n('green')}</span> <span class="pill blue">여유 ${n('blue')}</span> <span class="pill warn">제로 ROAS 모름 ${n('unknown')}</span> <span class="pill gray">광고비 없음 ${n('idle')}</span>
    <span class="sub" style="margin-left:10px">제로 ROAS: <span class="pill good">입력됨 ${setN}</span> <span class="pill warn">미입력(기본값) ${unsetN}</span></span> <label class="chk sub"><input type="checkbox" id="roas-only-unset" ${roasOnlyUnset ? 'checked' : ''}> 미입력만 보기</label>`
  ...
  const shownRows = roasOnlyUnset ? rows.filter((r) => r.zeroSource !== 'manual') : rows;
  t.innerHTML = `<thead><tr><th class="l">캠페인</th><th>상태</th><th>모드</th><th>노출<div class="sub">직전 대비</div></th><th>광고비</th><th>광고매출</th><th>ROAS<div class="sub">직전</div></th><th>제로 ROAS</th><th>광고센터 목표</th><th>광고 이익(추정)<div class="sub">직전 대비</div></th><th class="l">판단 · 추천</th></tr></thead><tbody>`
    + shownRows.map((r) => `<tr class="st-${r.status}" data-c="${esc(r.campaign)}"><td class="l">${esc(r.campaign)}</td><td>${pill(r)}</td>
      <td><select class="mode"><option value="normal" ${r.mode === 'normal' ? 'selected' : ''}>보통</option><option value="summer" ${r.mode === 'summer' ? 'selected' : ''}>여름 시즌</option><option value="winter" ${r.mode === 'winter' ? 'selected' : ''}>겨울 시즌</option><option value="season" ${r.mode === 'season' ? 'selected' : ''}>시즌(종료일 직접)</option><option value="off" ${r.mode === 'off' ? 'selected' : ''}>비시즌</option></select><input type="date" class="mode-end" value="${esc(r.modeEnd)}" title="시즌 종료일 (뒤로 갈수록 기준이 빡빡해집니다)" style="${r.mode === 'season' ? '' : 'display:none'}"><div class="sub">${esc(r.band.label)}</div></td>
      <td class="num">${fmtInt(r.impressions)}${dpct(r.trend?.impressions)}</td><td class="num">${fmtWon(r.spend)}${dpct(r.trend?.spend)}</td><td class="num">${fmtWon(r.revenue)}${dpct(r.trend?.revenue)}</td><td class="num"><b>${r.spend ? Math.round(r.roas * 100) + '%' : '-'}</b>${r.trend ? `<div class="sub">직전 ${Math.round(r.trend.roasPrev * 100)}%</div>` : ''}<div class="sub" title="날짜별 ROAS (최근 ${r.daily.length}일)">${r.daily.map((x) => `${x.date.slice(8)}일 ${x.spend ? Math.round(x.roas * 100) + '%' : '-'}`).join(' · ')}</div></td>
      <td class="num zero-cell ${r.zeroSource === 'manual' ? 'zero-set' : 'zero-unset'}"><b>${Math.round(r.zero * 100)}%</b> <button class="btn sm zero-edit" title="제로 ROAS 입력·변경 (시작일부터 적용)">✎</button><div class="sub">${r.zeroSource === 'manual' ? `<span class="pill good">입력됨</span> ${r.zeroFrom ? r.zeroFrom.slice(2) + '~' : '처음부터'}` : '<span class="pill warn">미입력 · 기본값</span>'}${r.zeroUpcoming?.length ? ` · ${r.zeroUpcoming.map((h) => `${h.from.slice(5)}부터 ${Math.round(h.value * 100)}% 예정`).join(', ')}` : ''}${r.calc?.zero ? ` · 계산 ${Math.round(r.calc.zero * 100)}%` : ''}</div>
        ...
        <details class="zero-margins" style="margin-top:4px;text-align:left"><summary class="sub" style="cursor:pointer">옵션 마진도 함께 바꾸기 (옵션 ${campMargins(r.campaign).n}개, 옵션마다 따로${(() => { const ds = campOptionRows(r.campaign).map((o) => o.since).filter((x) => x && x !== '처음부터').sort(); return ds.length ? ` · 마진 마지막 수정 ${ds[ds.length - 1]}` : ''; })()}) ▾</summary>
          <div class="row" style="gap:4px;flex-wrap:wrap;margin:4px 0"><span class="sub">한 번에 채우기:</span><input class="zm-delta num" type="number" step="1" placeholder="±원" style="width:70px" title="모든 옵션의 현재 마진에 이 금액을 더함(음수면 뺌)"><button class="btn sm zm-apply-delta">모두 ±원</button><input class="zm-pct num" type="number" step="0.1" placeholder="−%" style="width:60px" title="판매가(최근 60일 평균) 기준 % 만큼 마진에서 뺌. 할인율을 넣으세요"><button class="btn sm zm-apply-pct">판매가의 %만큼 빼기</button><span class="sub">비어 있는 칸은 그대로 둡니다</span></div>
          <table class="form-table zm-table"><thead><tr><th class="l">옵션ID</th><th class="l">옵션명</th><th>판매가(평균)</th><th>현재 마진</th><th class="l">적용 시작 (마지막 수정)</th><th>새 마진</th></tr></thead><tbody>
          ${campOptionRows(r.campaign).map((o) => `<tr data-oid="${esc(o.option_id)}" data-price="${o.price || 0}" data-cur="${o.margin}"><td class="l num">${esc(o.option_id)}</td><td class="l" style="max-width:320px;white-space:normal;line-height:1.3" title="${esc(o.full)}">${esc(o.name)}</td><td class="num sub">${o.price ? fmtWon(o.price) : '-'}</td><td class="num">${o.hist.length ? fmtWon(o.margin) : '<span class="pill warn">없음</span>'}</td><td class="l sub" title="${esc(o.hist.map((h) => `${h.effective_from || '처음부터'} ${fmtInt(h.margin)}원${h.note ? ' (' + h.note + ')' : ''}`).join('\n'))}">${o.since ? `${o.since === '처음부터' ? '처음부터' : o.since + '~'}${o.note ? ` · ${esc(o.note)}` : ''}${o.hist.length > 1 ? ` <span class="pill gray">이력 ${o.hist.length}</span>` : ''}` : ''}</td><td><input class="zm-new num" type="number" step="1" placeholder="${Math.round(o.margin)}" style="width:80px"></td></tr>`).join('') || '<tr><td colspan="6" class="sub">이 캠페인에 연결된 옵션이 없습니다</td></tr>'}
          </tbody></table></details>
  ...
  t.querySelectorAll('button.zero-save').forEach((b) => b.onclick = async () => {
    ...
    // 옵션 마진도 함께 (옵션마다 적은 값만, 같은 시작일로)
    let mgN = 0;
    for (const tr of td.querySelectorAll('tr[data-oid]')) { const txt = tr.querySelector('input.zm-new').value; if (txt === '') continue; const mg = parseNumber(txt) || 0; const id = tr.dataset.oid; if (fromAll) for (const h of S.marginHistory(DATA, id)) S.deleteMargin(DATA, id, h.effective_from); S.setMargin(DATA, id, mg, from, '제로 ROAS 와 함께 변경'); mgN++; }
    const mgNote = mgN ? ` · 옵션 ${mgN}개 마진 변경` : '';
  ...
  // 한 번에 채우기: 모두 ±원 / 판매가의 % 만큼 빼기 → 옵션별 새 마진 칸을 채워 준다 (저장 전에 하나씩 고칠 수 있음)
  t.querySelectorAll('button.zm-apply-delta').forEach((b) => b.onclick = () => { const box = b.closest('details'); const dv = parseNumber(box.querySelector('input.zm-delta').value); if (dv == null) return; box.querySelectorAll('tr[data-oid]').forEach((tr) => { tr.querySelector('input.zm-new').value = Math.round(Number(tr.dataset.cur) + dv); }); });
  t.querySelectorAll('button.zm-apply-pct').forEach((b) => b.onclick = () => { const box = b.closest('details'); const pc = parseFloat(box.querySelector('input.zm-pct').value); if (!(pc > 0)) return; let skipped = 0; box.querySelectorAll('tr[data-oid]').forEach((tr) => { const price = Number(tr.dataset.price); if (!price) { skipped++; return; } tr.querySelector('input.zm-new').value = Math.round(Number(tr.dataset.cur) - price * pc / 100); }); if (skipped) toast(`판매가를 모르는 옵션 ${skipped}개는 비워 두었습니다 (최근 60일 판매 없음)`, ''); });
  const dz = $('#roas-default'); if (dz && document.activeElement !== dz) dz.value = Math.round(S.zeroRoasDefault(DATA) * 100);
  const card = $('#roas-card'); if (card && !card._bound) { card._bound = true; try { card.open = localStorage.getItem('cc-roas-open') === '1'; } catch { card.open = false; } card.addEventListener('toggle', () => { try { localStorage.setItem('cc-roas-open', card.open ? '1' : '0'); } catch { /* 무시 */ } }); }
  const ou = $('#roas-only-unset'); if (ou) ou.onchange = () => { roasOnlyUnset = ou.checked; renderRoasCheck(); };
  const saveMode = async (tr) => { const c = tr.dataset.c; const mode = tr.querySelector('select.mode').value; const end = tr.querySelector('input.mode-end').value; S.setCampMode(DATA, c, mode, end); await S.save(DATA); renderRoasCheck(); toast(`${c}: ${({ season: '시즌' + (end ? ' (종료 ' + end + ')' : ''), summer: '여름 시즌', winter: '겨울 시즌', off: '비시즌' })[mode] || '보통'}`); };
...
function renderOptions() {
  ...
  const cc = $('#opt-camp-card'); cc.innerHTML = '';
  if (onlyCamp && !onlyCamp.startsWith('__')) {
    const card = campCard(d, onlyCamp, { onlyMissing: true }); const nCand = card.querySelectorAll('tbody tr').length - 1;
    const head = document.createElement('div'); head.className = 'sub'; head.style.margin = '0 0 4px';
    head.textContent = nCand ? `광고 보고서·광고센터 기준으로 이 캠페인이 광고하지만 아직 목록에 없는 옵션 ${nCand}개 — 마진을 넣고 추가하세요. 옵션이 더 있어야 하면 '쿠팡에서 옵션 가져오기'를 누르세요.` : `이 캠페인이 광고하는 옵션 중 목록에 없는 것은 아직 모릅니다. 옵션이 더 있어야 하면 '쿠팡에서 옵션 가져오기'를 눌러 광고센터의 상품 목록을 읽어 오세요.`;
    cc.appendChild(head); cc.appendChild(card); cc.style.display = '';
...
function campCard(d, c, { onlyMissing = false } = {}) {
  ...
  const opts = Object.values(cand).filter((o) => !onlyMissing || !listed.has(o.option_id)).sort((a, b) => b.spend - a.spend);
...
function renderNewCampaigns(d) {
  ...
  for (const c of camps) box.appendChild(campCard(d, c));
  return camps.length;
}
...
const SETTINGS = { salesUrl: 'https://wing.coupang.com/tenants/business-insight/sales-analysis?start_date={date}&end_date={date}', adsUrl: 'https://advertising.coupang.com/marketing/dashboard/sales', autoEnabled: false, autoTime: '13:00', waitSeconds: 12, fillMissingDays: 7, ownWindow: true, reportEnabled: true, adsReportUrl: '', adsAccount: '', sellerName: '', serverSync: false, server: 'http://127.0.0.1:8765' };
...
$$('[data-testurl]').forEach((b) => b.onclick = async () => {
  ...
  b.disabled = false;
  const accLine = r.account?.length ? `<div>화면에 보이는 이름 후보: ${esc(r.account.join(' / '))} — 내 계정 이름을 위 칸에 적어 두세요.</div>` : '';
  if (r.hint) $('#url-test').innerHTML = `<span style="color:#a52a2a">${esc(r.hint)}</span>` + accLine;
  else if (r.ok) $('#url-test').innerHTML = `<span style="color:#0a7a0a">읽었습니다</span> · ${r.rows ? `${r.rows}줄 · 열: ${esc(r.headers.join(', '))}` : esc(r.download || '')}${r.date ? ` · 화면 날짜 ${r.date}` : ''}` + accLine;
...
$('#report-now').onclick = async () => {
  await chrome.storage.sync.set({ adsReportUrl: $('#set-adsReportUrl').value.trim(), reportEnabled: $('#set-reportEnabled').checked, adsAccount: $('#set-adsAccount').value.trim() });
  msg('#set-msg', '광고 보고서 받는 중… (창이 떠서 광고센터 보고서 화면을 엽니다, 1~2분)'); $('#report-now').disabled = true;
  const r = await chrome.runtime.sendMessage({ type: 'collectReport' });
...
$('#ads-delete-day').onclick = async () => {
  ...
  if (!confirm(`${date} 광고 데이터 ${n}건(캠페인 ${Object.keys(d.ads[date]).slice(0, 5).join(', ')}${n > 5 ? ' …' : ''})을 모두 지울까요?`)) return;
  delete d.ads[date]; await S.save(d); await reload(); loadAds(); msg('#ads-msg', `${date} 광고 ${n}건 삭제`, 'ok');
};
