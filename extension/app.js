...
function renderRoasCheck() {
  ...
  t.innerHTML = `<thead><tr><th class="l">캠페인</th><th>상태</th><th>모드</th><th>노출<div class="sub">직전 대비</div></th><th>광고비</th><th>광고매출</th><th>ROAS<div class="sub">직전</div></th><th>제로 ROAS</th><th>광고센터 목표</th><th>광고 이익(추정)<div class="sub">직전 대비</div></th><th class="l">판단 · 추천</th></tr></thead><tbody>`
    + rows.map((r) => `<tr class="st-${r.status}" data-c="${esc(r.campaign)}"><td class="l">${esc(r.campaign)}</td><td>${pill(r)}</td>
      ...
      <td class="num"><input class="zero num" type="number" step="1" placeholder="${Math.round(r.zero * 100)}" value="${r.zeroSource === 'manual' ? Math.round(r.zero * 100) : ''}" title="비우면 기본값 ${Math.round(S.zeroRoasDefault(DATA) * 100)}% 를 씁니다. ${r.calc?.zero ? `옵션 마진으로 계산하면 ${Math.round(r.calc.zero * 100)}% (판매가 ${fmtWon(r.calc.price)}원 · 마진 ${fmtWon(r.calc.margin)}원 · 최근 30일 ${fmtInt(r.calc.qty)}개)` : '옵션 마진으로는 계산할 데이터가 없습니다'}">%<div class="sub">${r.zeroSource === 'manual' ? '직접 입력' : '기본값'}${r.calc?.zero ? ` · 계산 ${Math.round(r.calc.zero * 100)}%` : ''}</div></td>
    ...
    + '</tbody>';
  t.querySelectorAll('input.zero').forEach((inp) => inp.onchange = async () => { const c = inp.closest('tr').dataset.c; const v = parseFloat(inp.value); const dd = await reload(); S.setZeroRoas(dd, c, v > 0 ? v / 100 : null); await S.save(dd); await reload(); renderRoasCheck(); toast(`${c} 제로 ROAS ${v > 0 ? v + '% 저장' : '직접 입력 지움 (기본값 사용)'}`); });
