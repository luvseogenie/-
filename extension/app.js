...
function campOptionRows(c) {
  ...
  const opts = DATA.options.filter((o) => o.campaign === c); const ids = new Set(opts.map((o) => o.option_id)); const rev = {}, qty = {};
  ...
  return opts.map((o) => ({ option_id: o.option_id, name: o.product_name || names[o.option_id] || '', margin: lookup(o.option_id, t), price: qty[o.option_id] ? rev[o.option_id] / qty[o.option_id] : 0 }));
}
    ...
    + rows.map((r) => `<tr class="st-${r.status}" data-c="${esc(r.campaign)}"><td class="l">${esc(r.campaign)}</td><td>${pill(r)}</td>
      ...
      <td class="num zero-cell"><b>${Math.round(r.zero * 100)}%</b> <button class="btn sm zero-edit" title="제로 ROAS 입력·변경 (시작일부터 적용)">✎</button><div class="sub">${r.zeroSource === 'manual' ? (r.zeroFrom ? r.zeroFrom.slice(2) + '~' : '처음부터') : '기본값'}${r.calc?.zero ? ` · 계산 ${Math.round(r.calc.zero * 100)}%` : ''}</div>
        ...
        <details class="zero-margins" style="margin-top:4px;text-align:left"><summary class="sub" style="cursor:pointer">옵션 마진도 함께 바꾸기 (옵션 ${campMargins(r.campaign).n}개, 옵션마다 따로) ▾</summary>
          ...
          <table class="form-table zm-table"><thead><tr><th class="l">옵션ID</th><th class="l">상품명</th><th>판매가(평균)</th><th>현재 마진</th><th>새 마진</th></tr></thead><tbody>
          ${campOptionRows(r.campaign).map((o) => `<tr data-oid="${esc(o.option_id)}" data-price="${o.price || 0}" data-cur="${o.margin}"><td class="l num">${esc(o.option_id)}</td><td class="l" style="max-width:260px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap" title="${esc(o.name)}">${esc(o.name)}</td><td class="num sub">${o.price ? fmtWon(o.price) : '-'}</td><td class="num">${fmtWon(o.margin)}</td><td><input class="zm-new num" type="number" step="1" placeholder="${Math.round(o.margin)}" style="width:80px"></td></tr>`).join('') || '<tr><td colspan="5" class="sub">이 캠페인에 연결된 옵션이 없습니다</td></tr>'}
          </tbody></table></details>
